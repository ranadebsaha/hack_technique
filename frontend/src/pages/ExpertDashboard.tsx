...
const ExpertDashboard = () => {
  ...
  const getExpertData = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://65.20.81.114:5000/cyber-bandhu/expert/all`, {
        headers: {
  ...
  const getExpertServices = async () => {
    if (!expertId) return;
    try {
      const res = await fetch(`http://65.20.81.114:5000/cyber-bandhu/service/expert/${expertId}`, {
        headers: {
          authorization: `Bearer ${token}`,
        },
      });
  ...
  const getMyPendingServices = async () => {
    if (!expertId) return;
    try {
      const res = await fetch(`http://65.20.81.114:5000/cyber-bandhu/service/pending/${expertId}`, {
        headers: {
          authorization: `Bearer ${token}`,
        },
      });
  ...
  const fetchServiceDetails = async (id: string) => {
    try {
      const res = await fetch(`http://65.20.81.114:5000/cyber-bandhu/service/details/${id}`, {
        headers: {
          authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleAcceptService = async (serviceId: string) => {
    ...
    try {
      const res = await fetch(`http://65.20.81.114:5000/cyber-bandhu/service/update/${serviceId}`, {
        method: "PUT",
        headers: {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://65.20.81.114:5000/cyber-bandhu/expert/update/${expertId}`, { // Assuming endpoint exists
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          authorization: `Bearer ${token}`,
